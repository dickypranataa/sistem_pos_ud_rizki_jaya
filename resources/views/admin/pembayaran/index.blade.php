                ...
                <tr class="hover:bg-slate-50/70 transition-colors duration-150">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-2">
                            ...
                            <form id="delete-form-{{ $pembayaran->id }}" action="{{ route('admin.pembayaran.destroy', $pembayaran->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button type="button" onclick="confirmDelete({{ $pembayaran->id }}, '{{ addslashes($pembayaran->nama_pembayaran) }}')"
...
<script>
    function openModal() {
        ...
        document.getElementById('add_nama_pembayaran').value = '';
    }
    function closeModal() {
    ...
    function confirmDelete(id, namaPembayaran) {
        Swal.fire({
            title: 'Apakah Anda yakin?',
            text: "Metode pembayaran '" + namaPembayaran + "' akan dihapus permanen!",
            icon: 'warning',
