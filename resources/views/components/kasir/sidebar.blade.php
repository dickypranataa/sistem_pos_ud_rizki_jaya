<aside id="sidebar-menu"
    ...
    <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-0.5">
        ...
        <a href="{{ route('kasir.piutang.index') }}"
            class="flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-xl transition-all duration-200 group
           {{ request()->routeIs('kasir.piutang.*') 
              ? 'bg-blue-600 text-white shadow-md shadow-blue-200' 
              : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
            <span class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 transition-all duration-200
                {{ request()->routeIs('kasir.piutang.*') ? 'bg-white/20' : 'bg-gray-100 group-hover:bg-blue-50' }}">
                <svg class="w-4 h-4 {{ request()->routeIs('kasir.piutang.*') ? 'text-white' : 'text-gray-500 group-hover:text-blue-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
