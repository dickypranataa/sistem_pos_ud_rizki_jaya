            ...
            <table class="w-full text-sm text-left">
                <thead>
                    <tr class="bg-blue-50 border-b border-blue-100">
                        <th
                            class="px-5 py-4 font-semibold text-blue-700 text-xs uppercase tracking-wider whitespace-nowrap">
                            Kode Transaksi</th>
                        ...
                        <th class="px-5 py-4 font-semibold text-blue-700 text-xs uppercase tracking-wider text-right">Total
                        </th>
                        <th class="px-5 py-4 font-semibold text-blue-700 text-xs uppercase tracking-wider text-right">Bayar
                        </th>
                        <th class="px-5 py-4 font-semibold text-blue-700 text-xs uppercase tracking-wider text-right">
                            Kembali</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @foreach ($transaksi as $transaksis)
                        <tr class="hover:bg-gray-50/60 transition">
                            ...
                            <td class="px-5 py-3 text-gray-500 text-xs whitespace-nowrap">{{ $transaksis->waktu_transaksi }}
                            </td>
                            <td class="px-5 py-3">
                                <span
                                    class="px-2 py-1 bg-gray-100 text-gray-600 text-xs font-semibold rounded-lg">{{ Str::headline($transaksis->tipe_harga) }}</span>
                            </td>
                            <td class="px-5 py-3 text-right font-bold text-gray-900 text-xs whitespace-nowrap">
                                {{ 'Rp ' . number_format($transaksis->total_harga, 0, ',', '.') }}</td>
                            <td class="px-5 py-3 text-right text-gray-600 text-xs whitespace-nowrap">
                                {{ 'Rp ' . number_format($transaksis->bayar, 0, ',', '.') }}</td>
                            <td class="px-5 py-3 text-right text-emerald-600 font-semibold text-xs whitespace-nowrap">
                                {{ 'Rp ' . number_format($transaksis->kembalian, 0, ',', '.') }}</td>
                            <td class="px-5 py-3">
