<div class="flex flex-col lg:flex-row h-[calc(100vh-64px)] w-full bg-gray-50">
    ...
    <div class="w-full lg:w-2/3 flex flex-col p-4 sm:p-6 lg:border-r border-gray-200">
        ...
        @if (session()->has('error'))
            <div
                class="bg-red-50 border border-red-200 text-red-600 px-4 py-2 text-sm rounded-md mb-4 flex items-center">
            ...
            @forelse($produks as $produk)
                ...
                <div wire:key="produk-{{ $produk->id }}" wire:click="tambahKeKeranjang({{ $produk->id }})"
                    ...
                    <div class="p-3 flex flex-col h-full relative">
                        @if ($produk->stok == 0)
                            <span
                                class="absolute top-0 right-0 bg-red-100 text-red-700 text-[10px] px-1.5 py-0.5 rounded-bl font-semibold border-b border-l border-red-200">Habis</span>
                        @elseif($produk->stok < 5)
                            <span
                                class="absolute top-0 right-0 bg-amber-100 text-amber-700 text-[10px] px-1.5 py-0.5 rounded-bl font-semibold border-b border-l border-amber-200">Sisa
                                {{ $produk->stok }}</span>
                        @endif
    ...
    <div class="w-full lg:w-1/3 bg-white flex flex-col z-20">
        ...
        <div class="px-5 py-4 border-b border-gray-200 bg-gray-50">
            ...
            <select wire:model.live="tipe_harga"
                class="w-full py-1.5 pl-3 pr-8 text-sm border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500">
        ...
        <div class="flex-1 px-5 py-4 overflow-y-auto space-y-3">
            @forelse($keranjang as $index => $item)
                <div wire:key="cart-item-{{ $index }}"
                    class="flex flex-col py-3 border-b border-gray-100 last:border-0">
                    <div class="flex justify-between items-start mb-2">
                        <div class="pr-3">
                            <h4 class="font-medium text-gray-900 text-sm leading-tight">{{ $item['nama'] }}</h4>
                            <p class="text-[11px] text-gray-500 mt-0.5">@ Rp
        ...
        <div class="p-5 bg-gray-50 border-t border-gray-200">
            ...
            <div class="space-y-3 mb-5">
                <div>
                    <label class="block text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Metode
                        Pembayaran</label>
                    <select wire:model="pembayaran_id"
                        class="w-full py-2 pl-3 pr-8 text-sm border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500">
                ...
                <div>
                    <label class="block text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Uang
                        Diterima</label>
                    <div class="relative">
            ...
            <div
                class="flex justify-between items-center mb-5 px-3 py-2 rounded-md {{ $kembalian < 0 ? 'bg-red-50 text-red-700 border border-red-100' : 'bg-emerald-50 text-emerald-700 border border-emerald-100' }}">
        ...
        document.addEventListener('livewire:initialized', () => {
            @this.on('buka-struk', (event) => {
                ...
                window.open(event.url, '_blank',
                    'width=400,height=600,toolbar=no,scrollbars=yes,resizable=yes');
            });
        });
    </script>
</div>
